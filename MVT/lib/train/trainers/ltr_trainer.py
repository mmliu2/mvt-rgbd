...
from lib.test.evaluation.tracker_depthtrack import Tracker # TODO_EVAL
from lib.train.eval_ao.dataset import DatasetBuilder

from tqdm import tqdm

class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, use_amp=False, eval_dataset='got10k'):
        """
        args:
...
class LTRTrainerDepth(LTRTrainer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, eval_dataset='depthtrack', **kwargs)
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(tqdm(loader, total=len(loader)), 1):
            ...
            #     # 1. Initialize the depthtrack dataset
            #     TEST_DATASET = 'depthtrackmini' # TODOTODO 'depthtrack'
            ...
            #     # 3. Evaluate the model on got10k_val dataset. Erase if got10k_val directory already exists from the
            #     # previous iteration
            #     if os.path.exists(os.path.join(tmp_folder_path, TEST_DATASET)):
            #         shutil.rmtree(os.path.join(tmp_folder_path, TEST_DATASET))
            #     run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=1)
            ...
            # TEST EVAL
            # if (loader.training and i == len(loader) and self.epoch % 5 == 0): # or self.epoch == 1: 
                
            #     assert(len(self.test_loader) == 1)
            #     for i, test_data in enumerate(self.test_loader, 1):
            ...
            #         test_data['epoch'] = self.epoch
            #         test_data['settings'] = self.settings
            #         # forward pass
            #         if not self.use_amp:
            #             loss_test, stats_test = self.actor(test_data)
            #         else:
            #             with autocast():
            #                 loss_test, stats_test = self.actor(test_data)
