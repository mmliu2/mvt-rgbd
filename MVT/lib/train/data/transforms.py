...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:
            if image.shape[2] == 3:
                rgb = image[..., :3]
                img_gray = cv.cvtColor(rgb, cv.COLOR_RGB2GRAY)  # shape (H, W)
                img_gray_3 = np.stack([img_gray, img_gray, img_gray], axis=2)  # (H, W, 3)
                return img_gray_3  # (H, W, 3)
            elif image.shape[2] == 6:
                depth_channels = image[..., 3:]  # (H, W, 3)
                return np.concatenate([img_gray_3, depth_channels], axis=2)  # (H, W, 6)
            else:
