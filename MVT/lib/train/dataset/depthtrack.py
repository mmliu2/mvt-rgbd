# from https://github.com/xiaozai/DeT
...
class DepthTrack(BaseVideoDataset):
    ...
    def _build_sequence_list(self):
        ...
        file_path = os.path.join(ltr_path, 'data_specs', 'depthtrack_%s_split.txt'%self.split)
        sequence_list = pandas.read_csv(file_path, header=None) #, squeeze=True)
        if len(sequence_list.columns) == 1:
            sequence_list = sequence_list.squeeze("columns")
        sequence_list = sequence_list.values.tolist()
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        # image = (frame_list[0][:, :, :3])
        # bbox = anno_frames['bbox'][0]
