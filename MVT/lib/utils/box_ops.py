...
def box_pixelwise_metrics(boxes_pred, boxes_gt):
    pred_areas = box_area(boxes_pred)
    gt_areas = box_area(boxes_gt)

    lt = torch.max(boxes_pred[:, :2], boxes_gt[:, :2])
    rb = torch.min(boxes_pred[:, 2:], boxes_gt[:, 2:])
