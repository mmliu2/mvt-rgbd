...
from lib.models.mobilevit_track.mobilevit_det import MobileViTDeT
from lib.utils.box_ops import box_xyxy_to_cxcywh
from easydict import EasyDict as edict

class MobileViT_Track_DeT(nn.Module):
    """ This is the base class for MobileViT-Track """

    def __init__(self, backbone, backbone_dte, neck, neck_dte, feature_fusor, box_head, aux_loss=False, head_type="CORNER"): # TODO_DET
    # def __init__(self, backbone, neck, feature_fusor, box_head, aux_loss=False, head_type="CORNER"):
        ...
        self.backbone_dte = backbone_dte # TODO_DET
        if neck is not None:
            self.neck = neck
            self.neck_dte = neck_dte # TODO_DET
    ...
    def forward(self, template: torch.Tensor, search: torch.Tensor, template_dte: torch.Tensor, search_dte: torch.Tensor, train=True):
        template = torch.concat([template, template_dte], axis=1)
        search = torch.concat([search, search_dte], axis=1)

        x, z = self.backbone(x=search, z=template, train=train)
        x_dte, z_dte = self.backbone_dte(x=search, z=template, train=train) # TODO_DET

        # Forward neck
        x, z = self.neck(x, z)

        x_dte, z_dte = self.neck_dte(x, z) # TODO_DET

        assert x.shape == x_dte.shape
        assert z.shape == z_dte.shape
        x = torch.max(x, x_dte)
        z = torch.max(z, z_dte)
...
def build_mobilevit_track_det(cfg, training=False): # training=True):
    """
    ...
    if cfg.MODEL.BACKBONE.TYPE == 'mobilevit_s':
        backbone = create_mobilevit_det_backbone(pretrained, training=training)
        backbone_dte = create_mobilevit_det_backbone(pretrained, training=training) # TODO_DET
    ...
    backbone_dte.finetune_track(cfg=cfg, patch_start_index=patch_start_index) # TODO_DET
    ...
    if cfg.MODEL.NECK.TYPE == "FEATURE_FUSOR_HCAT":
        ...
        backbone_dte.position_embeddings = build_position_encoding(settings=pos_embed_settings) # TODO_DET

    # build neck module to fuse template and search region features
    if cfg.MODEL.NECK:
        neck = build_neck(cfg=cfg, hidden_dim=hidden_dim)
        neck_dte = build_neck(cfg=cfg, hidden_dim=hidden_dim) # TODO_DET
    ...
    model = MobileViT_Track_DeT(
        backbone,
        backbone_dte, # TODO_DET
        neck,
        neck_dte, # TODO_DET
        feature_fusor,
        box_head,
...
def create_mobilevit_det_backbone(pretrained, training=False):
    """
    ...
    model = MobileViTDeT(opts, training=training)
