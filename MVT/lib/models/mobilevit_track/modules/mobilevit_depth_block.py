...
class TransformerWithAdapter(nn.Module): # TODO_DELETE
    """
    Single transformer layer with adapters
    """
    def __init__(self, transformer_layer, dim, reduction=4, r=0.1):
        super().__init__()
        self.layer = transformer_layer
        self.adapter1 = Adapter(dim, reduction)
        self.adapter2 = Adapter(dim, reduction)
        self.r = r
        self.norm1 = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(dim)

    def forward(self, x, x_prev=None, attn_mask=None, key_padding_mask=None, *args, **kwargs):
        # Use x as default if x_prev is None
        if x_prev is None:
            x_prev = x 
        ...
        res = x
        x_norm = self.norm1(x)
        x_q = self.layer.pre_norm_mha[0](x_norm)
        x_attn = self.layer.pre_norm_mha[1](
            x_q=x_q,
            x_kv=x_prev,
            attn_mask=attn_mask,
        ...
        x_attn = self.layer.pre_norm_mha[2](x_attn)
        # Add adapter with small scaling factor
        x = res + x_attn + self.r * self.adapter1(x_attn)

        # ----- Feed-Forward Block -----
        x_norm2 = self.norm2(x)
        x_ffn = self.layer.pre_norm_ffn(x_norm2)
        # Adapter applied after FFN
        x = x + x_ffn + self.r * self.adapter2(x_ffn)

        return x
    
class MobileViTDepthBlock(BaseModule):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        global_rep = [
            TransformerWithAdapter(TransformerEncoder( # TODO_ADAPT
                opts=opts,
